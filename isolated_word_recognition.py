...
def gen_para_GMM(fea_collect,N_mix):
    ...
    for m in range(N_mix):
        index = np.where(labs == m)[0]#返回由第m个高斯分量所产生的数据对应的帧
        # print("----index---------",index)
        sub_feas = feas[index]
        mu = np.mean(sub_feas,axis=0)
        sigma = np.var(sub_feas,axis=0)
        sigma = sigma+0.0001#为了避免后续求解概率密度函数的时候方差太小，加上一个较小的数值
        mus[m] = mu
        sigmas[m] = sigma
        
        # print("------N  D-------",N,np.shape(index)[0])
        ws[m] = np.shape(index)[0]/N
    ws = (ws+0.01)/np.sum(ws+0.01) #计算3个高斯分量的权重，避免有的权重过小，所以加上了一个较小的数值
...
def init_hmm(collect_fea,N_state,N_mix):
    ...
    #建立一个空字典用于储存GMM-HMM，每个GMM-HMM包括了4个状态，每个状态包含三个高斯分量
    model_GMM_hmm = dict()
    ...
    for s in range(N_state):
        ...
        states.append(gmm)#储存每个状态的信息，每一个GMM是一个字典，由均值、方差和高斯权重表示
...
if __name__ == "__main__":
    ...
    for i in range(1,15):
        ...
        # 进入孤立词i所在的文件夹
        # 一共有14个孤立词，在文件夹中分别用数字命名
        wav_path = os.path.join(train_path,str(i))
        #使用os.path.join方法构建音频文件路径
        collect_fea = []
        len_feas = []
        #将./train/i文件夹下的.wav文件构建成一个可迭代对象
